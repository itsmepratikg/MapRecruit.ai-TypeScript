...
import { Search, Calendar, ChevronDown, Filter, MoreHorizontal, User, Clock, MapPin } from '../../../components/Icons';
...
export const GenericProfileList: React.FC<GenericProfileListProps> = ({ title, icon: Icon }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 dark:bg-slate-900 p-6 lg:p-8 overflow-hidden">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        ...
        <div className="flex items-center gap-3 bg-white dark:bg-slate-800 p-1 rounded-lg border border-slate-200 dark:border-slate-700 shadow-sm">
            <Calendar size={16} className="text-slate-400 ml-2" />
            <select 
                value={dateRange}
                onChange={(e) => setDateRange(e.target.value)}
                className="bg-transparent text-sm text-slate-700 dark:text-slate-200 font-medium focus:outline-none cursor-pointer py-1.5"
            >

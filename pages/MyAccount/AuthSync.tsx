...
import { 
  ...
  Save, RefreshCw, AlertCircle, Check, Key, Globe, LogOut, Edit2, AlertTriangle, Fingerprint
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <input 
          ...
          onChange={onChange}
          disabled={disabled}
          className={`w-full pl-3 pr-10 py-2.5 border rounded-lg text-sm outline-none transition-all ${
            disabled 
                    ...
                    <div className={`p-4 rounded-lg border flex items-center justify-between transition-colors ${ssoStatus.passkey ? 'bg-slate-50 dark:bg-slate-900/50 border-emerald-200 dark:border-emerald-900/30' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700'}`}>
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-slate-500 dark:text-slate-400">
                                    {ssoStatus.passkey ? <span className="text-emerald-600 dark:text-emerald-400 flex items-center gap-1"><CheckCircle size={10}/> Enabled</span> : 'Disabled'}
                                </p>
                            </div>
                        </div>
                        <label className={`relative inline-flex items-center ${isEditing ? 'cursor-pointer' : 'cursor-not-allowed opacity-60'}`}>
